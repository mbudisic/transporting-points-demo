        ...
        if pos_blobs_a and pos_blobs_b:
            ...
            for i, row_idx in enumerate(row_labels):
                ...
                for j, col_idx in enumerate(col_labels):
                    # Get the blob IDs from the labels with format "A{id} [x, y]" or "A{id} [height]"
                    row_id = int(row_idx.split(' ')[0][1:])  # Extract ID from "A{id}" part
                    col_id = int(col_idx.split(' ')[0][1:])  # Extract ID from "B{id}" part
